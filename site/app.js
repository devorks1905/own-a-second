...
var ERR = {
  ...
  MODERATED:'err_moderated', RATE_LIMITED:'err_rate_limited'
};
...
function openModal(c){
  ...
  var repBtn = $('modalReport');
  if (repBtn) { repBtn.onclick = function(){ reportClaim(c.id); }; }
...
function reportClaim(id){
  var reason = window.prompt(t('report_label') + ' — ' + t('reason_placeholder'));
  if (reason === null) return;
  fetch('/api/report', { method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ claimId: id, reason: reason || '' }) })
  .then(function(r){ return r.json(); })
  .then(function(res){ if (res.ok) alert(t('report_thanks')); })
  .catch(function(){});
}
